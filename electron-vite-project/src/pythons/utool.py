...
def process(
        ...
        data_json: str = Path(__file__).parent / "data" / "data.json",
        start_sec: int = 0,
        end_sec: int = None,
        interval: int = 1,
        scope: bool = False,
    ...
    """
    # --------------- 路径配置 ---------------
    video_path, data_json, out_dir, log_dir = Path(
        video_path), Path(data_json), Path(out_dir), Path(log_dir)
    ...
    if start_frame >= total_frames:
        raise RuntimeError(
            f"起始秒={start_sec} s 超出视频总长 {total_frames / fps:.1f} s")
    ...
    while current_frame_id < total_frames:
        ...
        for item in items:
            # 文字判断
            hit_rec = next(
                (rec for rec in ocr_records if item in rec[0]), None)
            ...
            cv2.imencode(".png", frame)[1].tofile(
                out_dir / f"仓库到账后_{item}.png")
            x, y, w, h = box
            cv2.rectangle(preview, (x, y), (w, h), (0, 255, 0), 2)
            truncated = f"{int(score * 1000) / 1000:.3f}"
            cv2.putText(preview, truncated, (x, y-6),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)

            # ⑤ 无乱码保存
            cv2.imencode(".png", preview)[1].tofile(
                out_dir / f"仓库到账后_{item}_预览.png")
            cv2.imencode(".png", frame)[1].tofile(
                out_dir / f"仓库到账后_{item}.png")
    ...
    logger.info("=" * 50)
    logger.info(
        f"OCR识别完成, 共命中 {len(hit_items)} 个物品: {hit_items}, 总耗时: {int(minutes):02d}分{seconds:05.2f}秒, 单次识别平均耗时: {total_time / frame_cnt:.2f} ms")
    cap.release()  # 释放视频句柄
