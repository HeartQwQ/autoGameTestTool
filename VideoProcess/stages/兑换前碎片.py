...
def exchange_shop_exchange_before(self, name, frame_res, img, logger, stage_dir):
    ...
    for (cleaned_text, score, box_poi, box) in frame_res:
        ...
        # 检查该碎片数是否在“兑换前”映射中
        if shard_num not in self.redeem_shards_before_map:
            continue

        # 根据兑换数量获取物品名称
        item_name = self.redeem_shards_before_map[shard_num]
